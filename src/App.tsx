...
export default function EcoApp() {
  ...
  useEffect(() => {
    if (!uid) return;
    const unsub1 = onSnapshot(collection(db, "memberTasks"), snap => {
    ...
    const unsub2 = onSnapshot(collection(db, "projects"), snap => {
      const ps = snap.docs.map(d => ({ id:d.id, ...d.data() })) as any[];
      if (ps.length > 0) setProjects(ps);
    });
    const unsub3 = onSnapshot(collection(db, "users"), snap => {
      const lb = snap.docs.map(d => ({ uid:d.id, ...d.data() }))
  ...
  async function toggleCh(id: number, pts: number) {
    ...
    if (uid) {
      ...
      await updateDoc(ref, { completed: Array.from(newSet), points: newPts });
    }
  ...
  async function doCheckin() {
    ...
    if (uid) {
      await updateDoc(doc(db,"users",uid), {
        checkinDays: Array.from(newDays), streak: newStreak, points: newPts,
      });
    }
  ...
  async function donate(projId: string, amt: number) {
    ...
    if (uid) await updateDoc(doc(db,"users",uid), { donations: newDon });
    showToast("💚 感謝你的贊助！");
    setDonateModal(null);
  }
  ...
  async function donateTask(taskId: string, amt: number) {
    ...
    if (uid) await updateDoc(doc(db,"users",uid), { taskDonations: newDon });
    showToast("💚 成功贊助任務！");
  }
  ...
  async function joinTask(taskId: string) {
    ...
    setMemberTasks(ts => ts.map(t => t.id===taskId ? { ...t, joined: isJoined ? t.joined.filter((j:string)=>j!==uid) : [...(t.joined||[]), uid] } : t));
    const ref = doc(db,"memberTasks",taskId);
    await updateDoc(ref, { joined: isJoined ? arrayRemove(uid) : arrayUnion(uid) });
  ...
  async function submitTask() {
    ...
    const ref = await addDoc(collection(db,"memberTasks"), task);
    setMemberTasks(ts => [...ts, { ...task, id: ref.id }]);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:500, padding:20 }} onClick={() => setShowProfile(false)}>
      <div style={{ background:"#fff", borderRadius:24, padding:28, width:"100%", maxWidth:340 }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ background:"#f9f9f9", borderRadius:14, padding:14, marginBottom:20, textAlign:"center" }}>
          ...
          <div style={{ fontSize:11, color:"#aaa", marginTop:2 }}>距下一等級還需 {level.next - points} 點</div>
        </div>
        {authUser && !isGuest ? (
    ...
    <div style={{ maxWidth:480, margin:"0 auto", background:"#f5f7f5", minHeight:"100vh", fontFamily:FF, position:"relative" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#1b5e20,#2e7d32)", padding:"16px 20px 20px", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <button onClick={() => setShowProfile(true)} style={{ background:"rgba(255,255,255,0.15)", border:"none", borderRadius:99, padding:"8px 14px", color:"#fff", cursor:"pointer", fontSize:22, display:"flex", alignItems:"center", gap:6 }}>
            {userAvatar}
          </button>
        </div>
        {/* 訪客提示 */}
        {isGuest && (
          <div style={{ marginTop:10, background:"rgba(255,204,2,0.2)", borderRadius:10, padding:"8px 12px", fontSize:12, color:"#ffcc02", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span>⚠️ 訪客模式，進度不會儲存</span>
            <button onClick={() => setIsGuest(false)} style={{ background:"#ffcc02", border:"none", borderRadius:8, padding:"4px 10px", fontSize:11, color:"#1b5e20", fontWeight:"bold", cursor:"pointer", fontFamily:FF }}>立即登入</button>
          </div>
        )}
        {/* Tab bar */}
      ...
      <div style={{ padding:"16px 16px 80px" }}>
        ...
        {tab === "挑戰" && (
          <div>
            <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:8, marginBottom:16 }}>
              {cats.map(c => (
                <button key={c} onClick={() => setFilter(c)} style={{ flexShrink:0, padding:"6px 14px", borderRadius:99, border:"none", background:filter===c?"#2e7d32":"#fff", color:filter===c?"#fff":"#555", fontWeight:filter===c?"bold":"normal", fontSize:12, cursor:"pointer", fontFamily:FF, boxShadow:"0 1px 4px rgba(0,0,0,0.08)" }}>{c}</button>
              ))}
            </div>
            {CHALLENGES.filter(c => filter==="全部" || c.category===filter).map(ch => {
              const done = completed.has(ch.id);
              return (
                <div key={ch.id} style={{ background:"#fff", borderRadius:18, padding:16, marginBottom:12, boxShadow:"0 2px 12px rgba(0,0,0,0.07)", border:done?"2px solid #2e7d32":"2px solid transparent", transition:"border 0.3s" }}>
                  <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
                    <div style={{ fontSize:34, minWidth:40 }}>{ch.icon}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        <div style={{ fontWeight:"bold", fontSize:15, color:"#1b3a1a" }}>{ch.title}</div>
                        <div style={{ fontWeight:"bold", color:"#2e7d32", fontSize:14 }}>+{ch.points}</div>
                      </div>
                      <div style={{ fontSize:12, color:"#888", marginTop:2 }}>{ch.desc}</div>
                      <div style={{ display:"flex", gap:6, marginTop:8, alignItems:"center" }}>
                        <span style={{ fontSize:11, background:CAT_COLORS[ch.category as keyof typeof CAT_COLORS]+"22", color:CAT_COLORS[ch.category as keyof typeof CAT_COLORS], padding:"2px 8px", borderRadius:99, fontWeight:"bold" }}>{ch.category}</span>
                        <span style={{ fontSize:11, background:DIFF_BADGE[ch.difficulty as keyof typeof DIFF_BADGE].bg, color:DIFF_BADGE[ch.difficulty as keyof typeof DIFF_BADGE].color, padding:"2px 8px", borderRadius:99 }}>{ch.difficulty}</span>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => toggleCh(ch.id, ch.points)} style={{ marginTop:12, width:"100%", padding:"10px", borderRadius:12, border:"none", background:done?"#e8f5e9":"linear-gradient(135deg,#2e7d32,#43a047)", color:done?"#2e7d32":"#fff", fontWeight:"bold", fontSize:14, cursor:"pointer", fontFamily:FF }}>
                    {done ? "✅ 已完成！" : "完成挑戰"}
                  </button>
                </div>
              );
        ...
        {tab === "打卡" && (
          <div>
            {/* 打卡按鈕 */}
            <div style={{ background:"linear-gradient(135deg,#1b5e20,#2e7d32)", borderRadius:22, padding:24, textAlign:"center", marginBottom:20, position:"relative", overflow:"hidden" }}>
              {checkinAnim && <div style={{ position:"absolute", inset:0, background:"rgba(255,255,255,0.15)", borderRadius:22, animation:"pulse 1.5s ease-out" }} />}
              <div style={{ fontSize:48, marginBottom:8 }}>{checkedIn ? "✅" : "📅"}</div>
              <div style={{ color:"#fff", fontWeight:"bold", fontSize:18, marginBottom:4 }}>{checkedIn ? "今日已打卡！" : "今日尚未打卡"}</div>
              <div style={{ color:"#a5d6a7", fontSize:13, marginBottom:16 }}>🔥 連續 {streak} 天</div>
              {!checkedIn && (
                <button onClick={doCheckin} style={{ background:"#fff", border:"none", borderRadius:14, padding:"12px 32px", fontWeight:"bold", color:"#2e7d32", fontSize:16, cursor:"pointer", fontFamily:FF }}>
                  立即打卡 +10pts
                </button>
              )}
            </div>
            {/* 月曆 */}
            <div style={{ background:"#fff", borderRadius:18, padding:16, boxShadow:"0 2px 12px rgba(0,0,0,0.07)" }}>
              <div style={{ fontWeight:"bold", fontSize:15, marginBottom:12, color:"#1b3a1a" }}>
                {yr}年{mo+1}月 打卡記錄
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:4 }}>
                ...
                {Array.from({length:dim}).map((_,i)=>{
                  ...
                  return (
                    <div key={d} style={{ textAlign:"center", padding:"6px 2px", borderRadius:8, background:checked?"#2e7d32":isToday?"#e8f5e9":"transparent", color:checked?"#fff":isToday?"#2e7d32":"#555", fontSize:12, fontWeight:checked||isToday?"bold":"normal" }}>
                      {d}
                    </div>
                  );
                })}
        ...
        {tab === "排行榜" && (
          <div>
            <div style={{ background:"linear-gradient(135deg,#1b5e20,#2e7d32)", borderRadius:20, padding:20, textAlign:"center", marginBottom:20 }}>
              <div style={{ fontSize:40 }}>🏆</div>
              <div style={{ color:"#fff", fontWeight:"bold", fontSize:18, marginTop:8 }}>環保英雄榜</div>
              <div style={{ color:"#a5d6a7", fontSize:13 }}>你的排名：#{(leaderboard.findIndex((u:any)=>u.uid===uid)+1)||"?"}</div>
            </div>
            {(leaderboard.length > 0 ? leaderboard : [
              {uid:"demo1",displayName:"環保達人",avatar:"🌍",points:520,streak:20},
              {uid:"demo2",displayName:"地球守護者",avatar:"🌳",points:380,streak:12},
              {uid:"demo3",displayName:"綠能小子",avatar:"⚡",points:240,streak:5},
            ]).map((u:any,i:number) => (
              <div key={u.uid} style={{ background:"#fff", borderRadius:16, padding:"14px 16px", marginBottom:10, display:"flex", alignItems:"center", gap:12, boxShadow:"0 2px 8px rgba(0,0,0,0.06)", border:u.uid===uid?"2px solid #2e7d32":"2px solid transparent" }}>
                <div style={{ fontWeight:"bold", fontSize:18, color:i===0?"#FFD700":i===1?"#C0C0C0":i===2?"#CD7F32":"#ccc", width:32, textAlign:"center" }}>{i===0?"🥇":i===1?"🥈":i===2?"🥉":"#"+(i+1)}</div>
                <div style={{ fontSize:28 }}>{u.avatar||"🌱"}</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:"bold", fontSize:14 }}>{u.displayName||"環保會員"}{u.uid===uid?" (我)":""}</div>
                  <div style={{ fontSize:11, color:"#aaa" }}>🔥 {u.streak||0}天</div>
                </div>
        ...
        {tab === "贊助" && (
          <div>
            {projects.map(p => {
              ...
              return (
                <div key={p.id} style={{ background:p.color||"#E8F5E9", borderRadius:20, padding:18, marginBottom:14, boxShadow:"0 2px 12px rgba(0,0,0,0.07)" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
                    <div style={{ fontSize:36 }}>{p.icon}</div>
                    <div>
                      <div style={{ fontWeight:"bold", fontSize:16, color:"#1b3a1a" }}>{p.title}</div>
                  ...
                  <div style={{ marginBottom:8 }}>
                    ...
                    <div style={{ background:"rgba(255,255,255,0.6)", borderRadius:99, height:8, overflow:"hidden" }}>
                      <div style={{ height:"100%", width:pct+"%", background:"linear-gradient(90deg,"+p.accent+"88,"+p.accent+")", borderRadius:99, transition:"width 0.6s" }} />
                    </div>
                  ...
                  {myAmt > 0 && <div style={{ fontSize:12, color:p.accent, fontWeight:"bold", marginBottom:8 }}>✅ 我已贊助 NT${myAmt}</div>}
        ...
        {tab === "發起" && (
          <div>
            <button onClick={() => setShowCreate(true)} style={{ width:"100%", padding:16, borderRadius:16, border:"none", background:"linear-gradient(135deg,#2e7d32,#43a047)", color:"#fff", fontWeight:"bold", fontSize:16, cursor:"pointer", fontFamily:FF, marginBottom:20 }}>
              ✨ 發起新環保任務
            </button>
            {memberTasks.map(task => {
              ...
              return (
                <div key={task.id} style={{ background:task.color||"#E8F5E9", borderRadius:20, padding:18, marginBottom:14, boxShadow:"0 2px 12px rgba(0,0,0,0.07)" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                    <div style={{ fontSize:32 }}>{task.icon}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:"bold", fontSize:15 }}>{task.title}</div>
                      <div style={{ fontSize:12, color:"#888" }}>{task.avatar} {task.creatorName} · {task.category} · 截止 {task.deadline}</div>
                    </div>
                  </div>
                  <div style={{ fontSize:13, color:"#555", marginBottom:10 }}>{task.desc}</div>
                  {task.location && mapOpen===task.id && (
                    <MiniMap lat={task.location.lat} lng={task.location.lng} label={task.location.name} accent={task.accent} />
                  )}
                  {task.location && (
                    <button onClick={() => setMapOpen(mapOpen===task.id?null:task.id)} style={{ fontSize:12, color:task.accent, background:"transparent", border:"none", cursor:"pointer", marginBottom:8, fontFamily:FF }}>
                  ...
                  {!done && (
                    <div style={{ display:"flex", gap:8 }}>
                      {[50,100,200].map(amt => (
                        <button key={amt} onClick={() => donateTask(task.id, amt)} style={{ flex:1, padding:"8px 0", borderRadius:10, border:"1.5px solid "+task.accent, background:"transparent", color:task.accent, fontWeight:"bold", fontSize:12, cursor:"pointer", fontFamily:FF }}>NT${amt}</button>
                      ))}
                  ...
                  {myAmt>0 && <div style={{ marginTop:8, textAlign:"right", fontSize:11, color:task.accent, fontWeight:"bold" }}>我已贊助 NT${myAmt}</div>}
                </div>
              );
            })}
          </div>
      ...
      {donateModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:300 }} onClick={() => setDonateModal(null)}>
          <div style={{ background:"#fff", borderRadius:"22px 22px 0 0", padding:"24px 20px 32px", width:"100%", maxWidth:480 }} onClick={e=>e.stopPropagation()}>
            ...
            <button onClick={() => setDonateModal(null)} style={{ width:"100%", marginTop:14, padding:12, borderRadius:12, border:"none", background:"#f5f5f5", color:"#888", fontWeight:"bold", cursor:"pointer", fontFamily:FF }}>取消</button>
          </div>
        </div>
      )}
          ...
          <div style={{ background:"#fff", borderRadius:"22px 22px 0 0", padding:"24px 20px 32px", width:"100%", maxWidth:480, maxHeight:"85vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                {TASK_ICONS.map(ic => (
                  <button key={ic} onClick={() => setNewTask(p=>({...p,icon:ic}))} style={{ width:40, height:40, borderRadius:10, border:"2px solid "+(newTask.icon===ic?"#2e7d32":"#e8f5e9"), background:newTask.icon===ic?"#e8f5e9":"#fafafa", fontSize:20, cursor:"pointer" }}>{ic}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:12 }}>
              ...
              <div>
                <div style={{ fontSize:12, color:"#555", fontWeight:"bold", marginBottom:6 }}>募資目標 (NT$) *</div>
                <input type="number" value={newTask.goal} onChange={e=>setNewTask(p=>({...p,goal:e.target.value}))} placeholder="5000" style={{ width:"100%", padding:"10px", borderRadius:12, border:"1.5px solid #e8f5e9", fontSize:13, fontFamily:FF, outline:"none", boxSizing:"border-box", background:"#fafafa" }} />
              </div>
            </div>
            <div style={{ marginBottom:20 }}>
            ...
            <button onClick={() => setShowCreate(false)} style={{ width:"100%", padding:12, borderRadius:12, border:"none", background:"#f1f8e9", color:"#888", fontWeight:"bold", cursor:"pointer", fontFamily:FF, fontSize:14 }}>取消</button>
          </div>
        </div>
      )}
